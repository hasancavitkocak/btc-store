    ...
    try {
      ...
      if (response.status === 'SUCCESS' && response.data) {
        ...
        if (pageData && pageData.content) {
          ...
          setToast({ message: t('usersAdmin.dataFormatError'), type: 'error' });
        }
      } else {
        setToast({ 
          message: response.errorMessage || t('usersAdmin.loadError'), 
          type: 'error' 
        });
      }
    } catch (error) {
      console.error('Error loading users:', error);
      setToast({ message: t('usersAdmin.loadError'), type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (code: string) => {
    try {
      ...
      if (response.status === 'SUCCESS') {
        setToast({ message: t('usersAdmin.deleteSuccess'), type: 'success' });
        setPage(1);
        if (page === 1) {
          loadUsers();
        }
      } else {
        setToast({ message: response.errorMessage || t('usersAdmin.deleteFailed'), type: 'error' });
      }
    } catch (error) {
      console.error('Error deleting user:', error);
      setToast({ message: t('usersAdmin.deleteError'), type: 'error' });
    }
  };
      ...
      <Container>
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">{t('usersAdmin.totalUsers', { count: totalElements })}</p>
          </div>
        ...
        {loading ? (
          <Card className="p-12 text-center">
            <p className="text-gray-500">{t('usersAdmin.loading')}</p>
          </Card>
        ) : (
          <>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        {t('usersAdmin.table.contact')}
                      </th>
                      <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                        {t('usersAdmin.table.groups')}
                      </th>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {user.userGroups && user.userGroups.length > 0 ? (
                              user.userGroups.slice(0, 2).map((ug, idx) => (
                                ...
                                >
                                  {getLocalizedText(ug.description, locale) || ug.code || t('usersAdmin.group')}
                                </span>
                              ))
                            ) : (
                              <span className="text-xs text-gray-400">-</span>
            ...
            {users.length === 0 && (
              <Card className="p-12 text-center mt-6">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div>
                    <p className="text-gray-900 font-medium mb-1">{t('usersAdmin.noUsers')}</p>
                    <p className="text-gray-500 text-sm mb-4">{t('usersAdmin.noUsersDesc')}</p>
                  </div>
                  <Button onClick={() => router.push('/admin/users/new')} className="bg-blue-600 hover:bg-blue-700">
                    <Plus className="w-4 h-4 mr-2" />
                    {t('usersAdmin.addFirstUser')}
                  </Button>
                </div>
              </Card>
            )}
            ...
            {totalElements > 0 && (
              <div className="mt-6 flex items-center justify-between border-t border-gray-200 pt-4">
                <div className="text-sm text-gray-600">
                  {t('usersAdmin.totalRecords', { count: totalElements })}
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600 px-4">
                    {t('usersAdmin.page')} <span className="font-medium">{page}</span> / <span className="font-medium">{totalPages || 1}</span>
                  </span>
        ...
        <ConfirmDialog
          ...
          title={t('usersAdmin.deleteDialog.title')}
          message={t('usersAdmin.deleteDialog.message', { username: deleteDialog.username })}
          confirmText={t('usersAdmin.deleteDialog.confirm')}
          cancelText={t('usersAdmin.deleteDialog.cancel')}
          type="danger"
        />
      </Container>
    </Section>
